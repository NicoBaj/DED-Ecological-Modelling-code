...
- `pre_roots_vs_routes.R` (where route is intended as the French for road) loads Openstreetmap (OSM) data for the geography: roads, parking lots, railroads and rivers. It computes the distance between each pair of trees in the elms file then sets up line segments between all trees that are less than some threshold maximum distance away (we use 6 times the tree heights). It then removes from this graph all edges that intersect at least one of the OSM objects. Two remarks about this script.
    - We compute distances in the entire city. The amount of RAM required for some of the operations is therefore quite substantial. One of the variables (that is cleared as soon as it not used anymore) weighs close to 80 GB. To run the code comfortably if other programs are running, 128 GB of RAM is therefore recommended.
    - There seems to have been a recent evolution in Linux's OOM-killer. If the code terminates with the comment `Killed`, this means the process is getting killed (by the Linux kernel). We are looking into this.
